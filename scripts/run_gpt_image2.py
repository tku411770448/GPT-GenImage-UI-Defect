#!/usr/bin/env python3
"""Run OpenAI GPT Image 2 API for industrial defect repair + relocation.

This script intentionally contains no local diffusion-model loading. It keeps the
original ROI/source-mask/target-area workflow, but calls OpenAI's Image Edit API.

Workflows:
  prompt-only-edit             image + prompt only, closest to ChatGPT UI upload + prompt
  repair-only                  source mask repairs original defect
  generate-only                target/fixed mask generates new defect(s)
  repair-and-random-generate   stage 1 repair source mask, stage 2 random masks inside target_area
"""
...
def default_chinese_prompt(defect_type: str, size: str = "1280x1280") -> str:
    ...
    return (
        f"這張圖的 Class Name 為 `{defect_type}`\n"
...
def load_user_prompt(defect_type: str | None, prompt_file: Path | None, inline_prompt: str | None, size: str) -> dict[str, str]:
    ...
    the ChatGPT UI style: ROI-annotated defect image + natural-language prompt.
    ...
    # Backward compatibility for older project prompt.txt files: previous
    # templates sometimes told the model to avoid black regions.  The UI now
    # treats Target Area as the only user-authorized placement constraint, so
    # remove only that stock sentence while keeping the user's custom wording.
    legacy_black_rules = {
        "不要將目標生成到黑色區域或 target area 之外。": "不要將目標生成到 target area 之外。",
        "不要將目標生成到黑色區域或 Target Area 之外。": "不要將目標生成到 Target Area 之外。",
        "不要生成到黑色區域或 target area 外。": "不要生成到 target area 外。",
        "不要生成到黑色區域或 Target Area 外。": "不要生成到 Target Area 外。",
        "不要生成到黑色區域或 target area 之外。": "不要生成到 target area 之外。",
        "不要生成到黑色區域或 Target Area 之外。": "不要生成到 Target Area 之外。",
    }
    for old_rule, new_rule in legacy_black_rules.items():
...
def build_prompt_bundle(cfg: dict[str, str]) -> dict[str, str]:
    defect_type = cfg.get("defect_type", "defect")
    user_prompt = cfg["user_prompt"].strip()
    # Restore GPTUI47 generation behavior: holistic image+prompt edit.
    # Avoid adding mask-stage wording when workflow=prompt-only-edit because it
    # tends to produce gray/black rectangular artifacts in this use case.
    base_note = (
        "\n\n補充要求：請把輸入圖像視為待編輯的工業瑕疵圖。"
        "若圖中有 ROI 框線或標註，僅作為定位參考，最終輸出不可保留框線或文字。"
    )
    prompt_only = user_prompt + base_note
    repair_prompt = (
        user_prompt
        + "\n\n此階段請優先修補 ROI / mask 中的原始瑕疵，使該位置變成乾淨且自然連續的背景。"
    )
    generation_prompt = (
        user_prompt
    ...
    single_pass_relocation = (
        ...
        + "\n- Treat the user-drawn Target Area as the sole valid random-placement source. Do not apply any automatic black-region, material-region, border, or surface filtering. If the Target Area includes black pixels, those pixels are still valid because the user selected them."
...
def _target_change_stats(before: Image.Image, after: Image.Image, target_area: Image.Image | None) -> dict[str, Any]:
    """Measure whether the model actually changed pixels inside Target Area.

    The edit model may accept a target-area prompt but still leave the region
    visually unchanged.  We use this only as a safety check; if the Target Area
    is effectively unchanged, a conservative ROI-derived defect stamp is added
    inside the user-drawn Target Area so the generated sample is not empty.
    """
    if target_area is None or target_area.getbbox() is None:
        return {"available": False, "changed_pixels": 0, "target_pixels": 0, "changed_ratio": 0.0}
    b = before.convert("RGB")
    a = after.convert("RGB")
    if a.size != b.size:
        a = a.resize(b.size, Image.Resampling.LANCZOS)
    t = binarize(target_area)
    diff = ImageChops.difference(b, a).convert("L")
    diff_np = np.array(diff, dtype=np.uint8)
    t_np = np.array(t, dtype=np.uint8) > 0
    target_pixels = int(t_np.sum())
    if target_pixels <= 0:
        return {"available": False, "changed_pixels": 0, "target_pixels": 0, "changed_ratio": 0.0}
    # Use a small but non-zero threshold so faint stains count, while global
    # JPEG/resize noise does not.
    changed_pixels = int(np.logical_and(t_np, diff_np >= 7).sum())
    ratio = float(changed_pixels) / float(max(1, target_pixels))
    return {"available": True, "changed_pixels": changed_pixels, "target_pixels": target_pixels, "changed_ratio": ratio}


def _make_soft_irregular_alpha(size: tuple[int, int], rng: random.Random, defect_type: str = "") -> Image.Image:
    w, h = size
    out = Image.new("L", (max(1, w), max(1, h)), 0)
    draw = ImageDraw.Draw(out)
    kind = str(defect_type or "").lower()
    cx, cy = w / 2.0, h / 2.0
    if "stripe" in kind:
        y = cy + rng.uniform(-h * 0.12, h * 0.12)
        thickness = max(2, int(h * rng.uniform(0.12, 0.22)))
        pts = [(1, int(y - thickness)), (w - 1, int(y - thickness + rng.randint(-2, 2))), (w - 1, int(y + thickness)), (1, int(y + thickness + rng.randint(-2, 2)))]
        draw.polygon(pts, fill=220)
    else:
        rx = max(2.0, w * rng.uniform(0.36, 0.50))
        ry = max(2.0, h * rng.uniform(0.30, 0.48))
        pts = []
        n = rng.randint(10, 18)
        for i in range(n):
            a = 2.0 * np.pi * i / n
            j = rng.uniform(0.58, 1.08)
            pts.append((int(cx + np.cos(a) * rx * j), int(cy + np.sin(a) * ry * j)))
        draw.polygon(pts, fill=210)
    radius = max(0.7, min(w, h) * 0.045)
    out = out.filter(ImageFilter.GaussianBlur(radius=radius))
    return out


def _sample_stamp_position_inside_target(
    stamp_alpha: Image.Image,
    target_area: Image.Image,
    rng: random.Random,
    attempts: int = 600,
) -> tuple[int, int] | None:
    target = np.array(binarize(target_area), dtype=np.uint8) > 0
    alpha_np = np.array(stamp_alpha, dtype=np.uint8) > 8
    if not alpha_np.any():
        return None
    H, W = target.shape
    sh, sw = alpha_np.shape
    if sw >= W or sh >= H:
        return None
    ...
    for _ in range(max(1, int(attempts))):
        # Draw around existing target pixels so thin / polygonal areas can still
        # be sampled, but require the actual alpha island to stay inside target.
        center_idx = rng.randrange(len(xs))
        cx, cy = int(xs[center_idx]), int(ys[center_idx])
        x = max(0, min(W - sw, cx - sw // 2 + rng.randint(-max(1, sw // 2), max(1, sw // 2))))
        y = max(0, min(H - sh, cy - sh // 2 + rng.randint(-max(1, sh // 2), max(1, sh // 2))))
        crop = target[y:y + sh, x:x + sw]
        if crop.shape == alpha_np.shape and np.all(crop[alpha_np]):
            return x, y
    return None


def ensure_target_defect_visible(
    source_image: Image.Image,
    generated_image: Image.Image,
    roi_mask: Image.Image | None,
    target_area: Image.Image | None,
    seed: int,
    min_defects: int,
    max_defects: int,
    ...
    """Guarantee target-area relocation when the API leaves Target Area empty.

    This is a targeted safety net for industrial-data augmentation.  It does not
    replace the OpenAI output when the target area already changed; it only adds
    small ROI-derived defect stamps inside the user-drawn Target Area when the
    model failed to create a visible target-area defect.
    """
    stats = _target_change_stats(source_image, generated_image, target_area)
    meta: dict[str, Any] = {"enabled": False, "target_change_stats": stats}
    if target_area is None or target_area.getbbox() is None or roi_mask is None or roi_mask.getbbox() is None:
        meta["reason"] = "missing ROI or Target Area mask"
        return generated_image, meta
    # If the model already produced a visible change in Target Area, keep it.
    if (not force) and stats.get("available") and (int(stats.get("changed_pixels", 0)) >= 18 and float(stats.get("changed_ratio", 0.0)) >= 0.00025):
        meta["reason"] = "target area already contains visible model changes"
        return generated_image, meta

    rng = random.Random(int(seed) + 91357)
    src = source_image.convert("RGB")
    out = generated_image.convert("RGB")
    if out.size != src.size:
        out = out.resize(src.size, Image.Resampling.LANCZOS)
    roi = binarize(roi_mask)
    tgt = binarize(target_area)
    bbox = roi.getbbox()
    if bbox is None:
        meta["reason"] = "empty ROI mask"
        return generated_image, meta

    roi_patch = src.crop(bbox)
    rw, rh = roi_patch.size
    if rw <= 1 or rh <= 1:
        meta["reason"] = "ROI patch too small"
        return generated_image, meta

    count = rng.randint(max(1, int(min_defects)), max(max(1, int(min_defects)), int(max_defects)))
    placements = []
    composite_mask = Image.new("L", src.size, 0)
    for idx in range(count):
        scale = rng.uniform(0.82, 1.16)
        sw = max(2, int(round(rw * scale)))
        sh = max(2, int(round(rh * scale)))
        # Shrink gradually if the user's target polygon is narrow.
        position = None
        patch = roi_patch.resize((sw, sh), Image.Resampling.LANCZOS)
        alpha = _make_soft_irregular_alpha((sw, sh), rng, defect_type)
        for shrink in [1.0, 0.85, 0.70, 0.55, 0.42, 0.32, 0.24]:
            if shrink != 1.0:
                sw2 = max(2, int(round(sw * shrink)))
                sh2 = max(2, int(round(sh * shrink)))
                patch = roi_patch.resize((sw2, sh2), Image.Resampling.LANCZOS)
                alpha = _make_soft_irregular_alpha((sw2, sh2), rng, defect_type)
            position = _sample_stamp_position_inside_target(alpha, tgt, rng)
            if position is not None:
                break
        if position is None:
            continue
        x, y = position
        layer = Image.new("RGB", src.size, (0, 0, 0))
        layer.paste(patch, (x, y))
        alpha_full = Image.new("L", src.size, 0)
        alpha_full.paste(alpha, (x, y))
        out = Image.composite(layer, out, alpha_full)
        composite_mask = ImageChops.lighter(composite_mask, alpha_full.point(lambda v: 255 if v > 8 else 0))
        placements.append({"index": idx + 1, "x": int(x), "y": int(y), "width": int(patch.size[0]), "height": int(patch.size[1]), "scale": float(round(scale, 4))})

    meta.update({
        "enabled": bool(placements),
        "reason": "OpenAI output had insufficient Target Area changes; added ROI-derived target-area defect stamp(s)" if placements else "fallback could not place ROI-derived stamp inside Target Area",
        "requested_defects": count,
        "placed_defects": len(placements),
        "placements": placements,
    })
    if run_dir is not None:
        try:
            composite_mask.save(run_dir / f"target_defect_fallback_mask_seed{seed}.png")
            out.save(run_dir / f"target_defect_fallback_preview_seed{seed}.png")
            (run_dir / f"target_defect_fallback_meta_seed{seed}.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
        except Exception:
            pass
    return out, meta
...
def parse_args():
    root = project_root()
    p = argparse.ArgumentParser(description="OpenAI GPT Image 2 industrial defect edit / relocation runner")
    ...
    p.add_argument("--image", type=Path, required=True, help="Input image path. Can be a plain defect image or an ROI-annotated image.")
    p.add_argument("--mask", type=Path, default=None, help="Optional source/repair mask. White pixels = original defect area to repair.")
    p.add_argument("--prototype-mask", type=Path, default=None, help="Optional prototype mask for random new defects. Defaults to --mask.")
    p.add_argument("--target-area", type=Path, default=None, help="Optional allowed new-defect placement mask. White pixels = valid generation area.")
    p.add_argument("--workflow", choices=["prompt-only-edit", "repair-and-random-generate", "repair-only", "generate-only"], default="prompt-only-edit")
...
def main():
    ...
    if args.prompt_extra.strip():
        for key in ["prompt_only_edit_prompt", "repair_prompt", "random_generation_prompt", "single_pass_relocation_prompt", "full_workflow_prompt"]:
    ...
    if args.workflow in {"repair-and-random-generate"} and args.target_area is None:
        raise ValueError("repair-and-random-generate requires --target-area")
    ...
    if target_area is not None and target_area.getbbox() is not None:
        # Target Area is the only user-authorized random-generation region.
        # Do not automatically exclude black/background/material regions, and do
        # not shrink it by subtracting the ROI; users can redraw Target Area if
        # they want a stricter region.  The repair ROI is still included in the
        # final edit mask for removal, but new random islands are sampled from
        # the original Target Area exactly as drawn.
        random_allowed_area = binarize(target_area)
    ...
    metadata: dict[str, Any] = {
        ...
        "seed_note": "Seed controls local mask placement only. OpenAI GPT Image API does not expose a deterministic seed parameter.",
    ...
    for i in range(int(args.num_outputs)):
        ...
        if args.workflow == "repair-and-random-generate":
            # Target-area guided relocation, corrected: use the full user-drawn
            # Target Area as the editable generation region.  Earlier builds
            # sampled tiny random islands inside the Target Area and sent only
            # those islands as the edit mask; gpt-image often interpreted that
            # as a small repair region and left it empty.  The user expects the
            # Target Area itself to define the random-placement range, so the
            # OpenAI mask is ROI + full Target Area.
            assert repair_mask is not None
            if target_area is None or target_area.getbbox() is None:
                raise ValueError("Target Area mask is empty. Please draw Target Area in Step 4.")
            target_full_mask = binarize(target_area)
            combined_mask = ImageChops.lighter(binarize(repair_mask), target_full_mask).point(lambda v: 255 if v > 0 else 0)
            placement_meta = {
                "mode": "full-target-area-guided",
                "seed": int(seed),
                "target_area_policy": "user_target_area_is_the_only_random_generation_range",
                "edit_mask_policy": "OpenAI edit mask = ROI repair area + full Target Area; no black/background filtering and no tiny-island clipping",
                "target_area_bbox_xyxy": list(map(int, target_full_mask.getbbox())) if target_full_mask.getbbox() else None,
                "roi_bbox_xyxy": list(map(int, binarize(repair_mask).getbbox())) if binarize(repair_mask).getbbox() else None,
                "requested_defects_from_prompt": f"{args.min_defects}-{args.max_defects}",
            }
            random_mask_path = run_dir / f"target_area_full_mask_seed{seed}.png"
            combined_mask_path = run_dir / f"single_pass_roi_full_target_mask_seed{seed}.png"
            target_full_mask.save(random_mask_path)
            combined_mask.save(combined_mask_path)
            make_mask_preview(current_image, target_full_mask, color=(0, 200, 255)).save(run_dir / f"target_area_full_mask_preview_seed{seed}.png")
            make_mask_preview(current_image, combined_mask, color=(255, 180, 0)).save(run_dir / f"single_pass_roi_full_target_mask_preview_seed{seed}.png")
            make_openai_edit_mask(combined_mask).save(run_dir / f"openai_roi_full_target_alpha_mask_seed{seed}.png")
            metadata["placement_records"].append(placement_meta)
            print(f"[INFO] full Target Area generation mask seed={seed}: {placement_meta}")
            print("[INFO] single-pass repair + full Target Area generation via one OpenAI Image Edit API call")

            # Add direct wording for this corrected mask semantics.  This keeps
            # Step 5 text intact but prevents the model from interpreting Target
            # Area as a forbidden / background region.
            target_prompt = (
                bundle.get("user_prompt") or bundle.get("single_pass_relocation_prompt") or bundle.get("prompt_only_edit_prompt") or ""
            ).strip()
            target_prompt += (
                "\n\n重要補充：此 API 呼叫的透明 mask 包含兩種可編輯範圍：ROI 修補區，以及使用者在 Step 4 畫出的完整 Target Area。"
                "請在同一張輸出圖中同時完成：1) 修補 ROI 位置；2) 在完整 Target Area 內自行選擇隨機位置生成相似瑕疵。"
                "Target Area 是唯一的隨機生成範圍；請不要套用黑色區域、材質區、邊界或表面篩選。"
                "即使 Target Area 位於黑色或深色背景，也必須視為有效生成範圍。"
                "請不要把整個 Target Area 填滿，只需在其中生成指定數量的相似瑕疵。"
            )
            ...
            generated, api_meta = _call_openai_image_edit(
                ...
                args,
                run_dir,
                f"single_pass_full_target_seed{seed}",
            ...
            # Safety net: if the model left Target Area essentially unchanged,
            # add a small ROI-derived defect stamp strictly inside Target Area.
            # This keeps the pipeline useful for dataset generation instead of
            # silently returning an image with no relocated defect.
            generated, fallback_meta = ensure_target_defect_visible(
                current_image,
                generated,
                repair_mask,
                target_full_mask,
            ...
            api_meta["target_area_visibility_safety_net"] = fallback_meta
            metadata.setdefault("target_area_visibility_safety_net", []).append({"seed": seed, **fallback_meta})
            print(f"[INFO] target-area visibility safety net: {fallback_meta}")
