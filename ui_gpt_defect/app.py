...
class MainWindow(QMainWindow):
    ...
    def page_prompt(self) -> QWidget:
        ...
        self.prompt_template_label = QLabel("模板樣式"); self.prompt_template_combo = QComboBox(); self.prompt_template_combo.addItems(["瑕疵"]); self.prompt_template_combo.setCurrentText(self.state.prompt_template)
    ...
    def region_status_text(self, p: Path) -> str:
        reg=parse_region_txt(self.regions_dir()/f"{p.stem}.txt")
        roi_count=sum(1 for k in reg.keys() if k.startswith("ROI"))
    ...
    def all_region_ready_image_paths(self) -> list[Path]:
        ...
        for p in list_images(self.inputs_dir()):
            ...
            reg = parse_region_txt(reg_path)
            if any(k.startswith("ROI") for k in reg.keys()) and parse_target_areas(reg_path):
                ready.append(p)
        return ready
    ...
    def save_current_regions(self, silent: bool=False) -> None:
        ...
        for idx, roi in enumerate(getattr(self.region_canvas, "rois", []), start=1):
            key = "ROI" if idx == 1 else f"ROI_{idx}"
            lines.append(key + ": " + ",".join(map(str, roi)))
        ...
        lines.append("note: ROI is the original edit/remove area; target_area entries are unioned allowed random generation areas.")
        ...
        try:
            ...
            if getattr(self.region_canvas, "rois", []):
                mask=Image.new("L",size,0); draw=ImageDraw.Draw(mask)
                for roi in self.region_canvas.rois: draw.rectangle(roi, fill=255)
                mask.save(roi_mask_path)
            elif roi_mask_path.exists():
    ...
    def build_generation_cmd(self) -> list[str]:
        ...
        cmd=[
            ...
            "--workflow","repair-and-random-generate",
